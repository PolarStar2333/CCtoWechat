...
回复格式: [字母][数字][字母][数字]...  多选可连写数字  如 A123B12  不加分隔符  /submit 提交  /stop 取消
```

**回复格式：**
- `[字母][数字][字母][数字]...` 字母递增，每个数字字符=一个选项号
- 多选可连写数字，如 `A123B1` = A 选 1/2/3 + B 选 1
- 类之间不加分隔符：`A123B12` 不是 `A123,B12` 或 `A123 B12`
- `2` — 纯数字映射到第一个问题
...
| `/hwpush-on"授权码"` | 开启华为负一屏推送（超10条自动推送完整回复） |
